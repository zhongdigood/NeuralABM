...
def generate_smooth_data(
    ...
    **__,
    ...
    """
    Generates a dataset of Damper-counts by iteratively solving the system of differential equations.
    """
...
def get_Damper_data(*, data_cfg: dict, h5group: h5.Group, write_init_state: bool = True):
    """Returns the training data for the Damper model. If a directory is passed, the
    ...
    if "load_from_dir" in data_cfg.keys():
        with h5.File(data_cfg["load_from_dir"], "r") as f:
            data = np.array(f["Damper"]["true_counts"])
